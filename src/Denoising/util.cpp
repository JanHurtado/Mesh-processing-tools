...
void getAllFaceAreas(TriMesh &mesh, vector<num_t> &areas)
{
    ...
    for(TriMesh::FaceIter f_it = mesh.faces_begin(); f_it != mesh.faces_end(); f_it++)
    {
        ...
        num_t S = 0.5 * (edge1 % edge2).length();
        areas[(*f_it).idx()] = S;
    }
}
...
void getFaceVertexAngle(TriMesh &mesh,TriMesh::FaceHandle fh,TriMesh::VertexHandle vh, num_t & angle)
{
    ...
    //num_t s = ( v1.length() + v2.length() + v3.length())/2;

    //area=sqrt( s*(s-v1.length()) * (s-v2.length()) * (s-v3.length()) ); //heron's formula
